...
aCursor.execute(
    "SELECT unit_nm, com_req, req, imp, clk, earn, exchange, fee, sdate FROM \"data_ocean\".\"report_daily\" WHERE date_parse(sdate, '%Y-%m-%d') >= cast(current_date - interval '90' day as timestamp);")
...
while index < len(iplists):
    sCursor.execute("INSERT INTO loctable(ip, longitude, latitude) VALUES (%s, %s, %s);",
                    (ipformatlist[index], longlist[index], latlist[index]))
    index += 1
